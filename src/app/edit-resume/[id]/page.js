...
export default function EditResume() {
  ...
  useEffect(() => {
    if (resume) {
      ...
      setEmploymentTypes(resume.employmentTypes?.map((et) => et.id));
  ...
  let eds = education?.map((ed) => {
    ...
    return { ...ed, end_date: end.getFullYear() };
  });
      ...
      <div className="container ptb7">
        ...
        <fieldset className={"fieldset fieldset-sm"}>
          ...
          <div className="radio-group">
            <div className="radio">
              {resume.gender === "Мужской" && <input type="radio" name="gender" id="g1" value={"Мужской"} onChange={handleGenderChange} checked />}
              {!resume.gender ||
                (resume.gender !== "Мужской" && <input type="radio" name="gender" id="g1" value={"Мужской"} onChange={handleGenderChange} />)}
              <label>Мужской</label>
            </div>
            <div className="radio">
              {resume.gender === "Женский" && <input type="radio" name="gender" id="g2" value={"Женский"} onChange={handleGenderChange} checked />}
              {!resume.gender ||
                (resume.gender !== "Женский" && <input type="radio" name="gender" id="g2" value={"Женский"} onChange={handleGenderChange} />)}
              <label>Женский</label>
            </div>
          </div>
        </fieldset>
        ...
        <AutoCompleteTags
          ...
          selected={skills ? skills.split(",").map((item) => ({ name: item })) : []}
        />
        <h3>Образование</h3>
        <AddEducation
          onChange={(eds) => {
